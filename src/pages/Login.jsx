...
// import { useTheme } from "../context/themeContext";
// import { useNavigate } from "react-router-dom";
// import { GoogleLogin } from '@react-oauth/google'; // Added Google Login Import
// import { Mail, Lock, Eye, EyeOff, ArrowRight, Code, Sun, Moon, Loader2, AlertCircle, CheckCircle2 } from 'lucide-react';
...
//   const [showPassword, setShowPassword] = useState(false);
//   const [isSignUp, setIsSignUp] = useState(false);
//   const [isForgotPassword, setIsForgotPassword] = useState(false); // Added view state for Forgot Password
...
//   // Handles standard Email/Password Sign In and Sign Up
//   const handleAuth = async (e) => {
//     e.preventDefault();
//     setLoading(true);
//     setMessage(null);
...
//   // Handles the Email Submission for Password Reset Links
...
//       setMessage({ type: 'success', text: response.data.message || 'Reset email sent! Please check your inbox.' });
//     } catch (err) {
...
//   // Handles verification and login via Google Tokens
...
//   // UI styling assignments matching your theme configs
//   const themeClasses = darkMode ? 'bg-slate-900' : 'bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50';
//   const cardClasses = darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-blue-100';
//   const textMain = darkMode ? 'text-white' : 'text-slate-900';
//   const textMuted = darkMode ? 'text-slate-400' : 'text-slate-500';
...
//             /* VIEW 2: STANDARD REGISTER & LOGIN FORM */
//             <>
//               <form onSubmit={handleAuth} className="space-y-5">
//                 <div className="space-y-1.5">
//                   <label className={`text-sm font-medium ml-1 ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}>
...
//               {/* --- THIRD PARTY AUTH SECTION: GOOGLE --- */}
//               <div className="mt-6 flex items-center justify-center space-x-2">
//                 <div className={`flex-1 h-px ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`}></div>
//                 <span className={`text-xs uppercase font-medium ${darkMode ? 'text-slate-500' : 'text-slate-400'}`}>Or continue with</span>
//                 <div className={`flex-1 h-px ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`}></div>
//               </div>
...
//               <div className="mt-5 flex justify-center w-full">
//                 <GoogleLogin
//                   // 1. ADD THIS LINE: It changes the button text based on the view
...
const CODE_SNIPPETS = [
  "O(n log n)", "two-sum", "BFS/DFS", "dp[i][j]", "hash map",
  "O(1) space", "binary search", "stack.push()", "left, right",
  "return memo[n]", "while(l<r)", "graph.adj[]",
];
...
const BgFloaters = () => (
  <div className="absolute inset-0 overflow-hidden pointer-events-none select-none z-0" aria-hidden>
    ...
    {/* Accent orbs */}
    <div className="login-orb absolute w-96 h-96 -top-20 -left-20"
      style={{ background: "var(--color-accent, #3b82f6)", filter: "blur(100px)", opacity: 0.3 }} />
    <div className="login-orb absolute w-72 h-72 bottom-10 right-10"
      style={{ background: "#06b6d4", animationDelay: "2s", filter: "blur(100px)", opacity: 0.3 }} />
    <div className="login-orb absolute w-48 h-48 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
      style={{ background: "#8b5cf6", animationDelay: "1s", opacity: 0.12, filter: "blur(80px)" }} />
  </div>
);
...
const LoginPage = () => {
  ...
  return (
    <div className={`relative overflow-hidden min-h-screen flex items-center justify-center p-4 transition-colors duration-300 ${themeClasses}`}>
      ...
      {/* Main Card */}
      <div className={`relative z-10 w-full max-w-md rounded-2xl border shadow-xl overflow-hidden transition-all duration-300 ${cardClasses}`}>
        <div className="p-8 pb-6 text-center">
          <div className="mx-auto w-16 h-16 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl flex items-center justify-center shadow-lg mb-6 transform rotate-3 hover:rotate-0 transition-all duration-300">
            <Code className="h-8 w-8 text-white" />
          </div>
