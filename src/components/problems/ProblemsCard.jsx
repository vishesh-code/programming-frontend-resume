...
// import React, { useState } from "react";
// import { Code, CheckCircle, Clock, Eye, EyeOff, Copy, Target, Loader2, Trash2, Edit, ChevronLeft, ChevronRight } from "lucide-react";
// import { useTheme } from "../../context/themeContext";
// import apiClient from "../../utils/apiClient"; 
...
// const ProblemsCard = ({ problem, onUpdate, onEdit, onDelete }) => {
//   const { darkMode } = useTheme();
...
//   // 🔥 NEW: Track the currently active solution index in the carousel
...
//     ? problem.solutions
//     : problem.solution 
//       ? [{ title: "Main Solution", code: problem.solution, language: "javascript" }] 
//       : [];

//   const handleNextSol = () => {
//     setActiveSolIndex((prev) => Math.min(solutionsToRender.length - 1, prev + 1));
//     setIsCopied(false);
//   };

//   const handlePrevSol = () => {
//     setActiveSolIndex((prev) => Math.max(0, prev - 1));
//     setIsCopied(false);
//   };

//   const toggleExpand = () => {
//     setIsExpanded(!isExpanded);
//     if (!isExpanded) setActiveSolIndex(0); // Reset carousel when opening
//   };

//   const currentSolution = solutionsToRender[activeSolIndex];
...
//             <div className="flex items-center space-x-2 sm:ml-4 shrink-0">
//               {onEdit && (
//                 <button onClick={() => onEdit(problem)} className={`p-2 rounded-lg transition-all duration-200 ${darkMode ? "text-slate-400 hover:bg-slate-700 hover:text-white" : "text-slate-500 hover:bg-slate-100 hover:text-slate-900"}`} title="Edit problem">
//                   <Edit className="h-4 w-4" />
//                 </button>
//               )}
              
//               <button onClick={handleDelete} disabled={isDeleting} className={`p-2 rounded-lg transition-all duration-200 text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 ${isDeleting ? "opacity-50 cursor-not-allowed" : ""}`} title="Delete problem">
//                 {isDeleting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
//               </button>

//               <button onClick={handleToggleSolved} disabled={isToggling} className={`p-2 rounded-lg transition-all duration-200 ${isToggling ? "opacity-70 cursor-not-allowed" : ""} ${problem.solved ? "bg-emerald-100 text-emerald-600 hover:bg-emerald-200" : darkMode ? "bg-slate-700/50 text-slate-400 hover:bg-blue-900/30 hover:text-blue-400 border border-slate-600" : "bg-slate-100 text-slate-400 hover:bg-blue-50 hover:text-blue-600 border border-slate-200"}`} title={problem.solved ? "Mark as incomplete" : "Mark as complete"}>
//                 {isToggling ? <Loader2 className="h-4 w-4 animate-spin" /> : problem.solved ? <CheckCircle className="h-4 w-4" /> : <Clock className="h-4 w-4" />}
//               </button>
              
//               <button onClick={toggleExpand} className="flex items-center space-x-1 sm:space-x-2 px-3 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg hover:from-blue-700 hover:to-indigo-700 transition-all duration-200 text-xs font-medium">
//                 {isExpanded ? (
//                   <><EyeOff className="h-3.5 w-3.5" /><span className="hidden sm:inline">Hide</span></>
//                 ) : (
//                   <><Eye className="h-3.5 w-3.5" /><span className="hidden sm:inline">Solution</span></>
...
//             <div className={`flex items-center space-x-4 text-xs font-mono ${darkMode ? "text-slate-400" : "text-slate-500"}`}>
//                {problem.time_complexity && <div className="flex items-center space-x-1"><Clock className="h-3 w-3" /><span>{problem.time_complexity}</span></div>}
//                {problem.space_complexity && <div className="flex items-center space-x-1"><Target className="h-3 w-3" /><span>{problem.space_complexity}</span></div>}
//             </div>
//           </div>
//         </div>
//       </div>

//       {/* Expanded Solutions View (Carousel format) */}
//       {isExpanded && (
//         <div className={`border-t ${darkMode ? "border-slate-700 bg-slate-800/50" : "border-blue-100 bg-blue-50/50"}`}>
//           <div className="p-3 sm:p-4">
...
//                     {/* Sliding Controls (Only show if multiple solutions exist) */}
...
//                         <button 
//                           onClick={handlePrevSol} 
//                           disabled={activeSolIndex === 0} 
//                           className={`p-1 rounded-md transition-colors ${activeSolIndex === 0 ? "opacity-30 cursor-not-allowed" : darkMode ? "hover:bg-slate-700 text-slate-300" : "hover:bg-slate-100 text-slate-600"}`}
//                         >
//                           <ChevronLeft className="w-4 h-4" />
//                         </button>
                        
//                         <span className={`text-[11px] font-mono font-medium ${darkMode ? "text-slate-400" : "text-slate-500"}`}>
//                           {activeSolIndex + 1} / {solutionsToRender.length}
//                         </span>
                        
//                         <button 
//                           onClick={handleNextSol} 
//                           disabled={activeSolIndex === solutionsToRender.length - 1} 
//                           className={`p-1 rounded-md transition-colors ${activeSolIndex === solutionsToRender.length - 1 ? "opacity-30 cursor-not-allowed" : darkMode ? "hover:bg-slate-700 text-slate-300" : "hover:bg-slate-100 text-slate-600"}`}
//                         >
...
//                       className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-xs font-medium ${isCopied ? "bg-emerald-100 text-emerald-700 border-emerald-200" : darkMode ? "bg-slate-700 text-slate-300 hover:bg-slate-600 border-slate-600" : "bg-white text-slate-700 border border-slate-200 hover:bg-slate-50"}`}
//                     >
//                       <Copy className="h-3.5 w-3.5" />
...
//                 <pre className={`p-3 rounded-xl overflow-x-auto text-xs font-mono border ${darkMode ? "bg-slate-900 text-slate-300 border-slate-700" : "bg-slate-900 text-slate-200 border-slate-800"}`}>
//                   <code>{currentSolution.code || "// Empty logic block"}</code>
//                 </pre>
//               </div>
//             )}
...
const ProblemsCard = ({ problem, onUpdate, onEdit, onDelete }) => {
  ...
  const [isExpanded, setIsExpanded] = useState(false);
  const [activeTab, setActiveTab] = useState("solutions"); // "solutions" | "ai"
  const [isCopied, setIsCopied] = useState(false);
  const [isToggling, setIsToggling] = useState(false); 
  const [isDeleting, setIsDeleting] = useState(false);
  ...
  const [isChatExpanded, setIsChatExpanded] = useState(false); // 🔥 NEW: Track chat height expansion
  ...
  const toggleExpand = () => {
    ...
    if (!isExpanded) {
      ...
      setIsChatExpanded(false); // Reset expansion when closed
    }
  };
    ...
    <div className={`${darkMode ? "bg-slate-800 border-slate-700" : "bg-white border-blue-100"} rounded-xl border shadow-sm hover:shadow-md transition-all duration-300 w-full overflow-hidden`}>
      <div className="p-3 sm:p-4">
        <div className="space-y-3">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
            <div className="flex flex-col items-start gap-1.5 flex-1 min-w-0">
              <h3 className={`text-base sm:text-lg font-bold truncate w-full ${darkMode ? "text-white" : "text-slate-900"}`}>{problem.question}</h3>
              <div className={`flex items-center space-x-1.5 px-2 py-0.5 rounded-full text-[11px] font-semibold border w-max ${difficultyStyle.bg} ${difficultyStyle.text} ${difficultyStyle.border}`}>
                <div className={`w-1.5 h-1.5 ${difficultyStyle.dot} rounded-full`}></div>
                <span>{problem.difficulty}</span>
              </div>
            ...
            <div className="flex items-center space-x-2 sm:ml-4 shrink-0">
              {onEdit && <button onClick={() => onEdit(problem)} className={`p-2 rounded-lg transition-all duration-200 ${darkMode ? "text-slate-400 hover:bg-slate-700 hover:text-white" : "text-slate-500 hover:bg-slate-100 hover:text-slate-900"}`}><Edit className="h-4 w-4" /></button>}
              <button onClick={handleDelete} disabled={isDeleting} className={`p-2 rounded-lg transition-all duration-200 text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10`}><Trash2 className="h-4 w-4" /></button>
              <button onClick={handleToggleSolved} disabled={isToggling} className={`p-2 rounded-lg transition-all duration-200 ${problem.solved ? "bg-emerald-100 text-emerald-600 hover:bg-emerald-200" : darkMode ? "bg-slate-700/50 text-slate-400 hover:bg-blue-900/30 hover:text-blue-400" : "bg-slate-100 text-slate-400 hover:bg-blue-50"}`}>{problem.solved ? <CheckCircle className="h-4 w-4" /> : <Clock className="h-4 w-4" />}</button>
      ...
      {isExpanded && (
        <div className={`border-t flex flex-col ${darkMode ? "border-slate-700 bg-slate-800/50" : "border-blue-100 bg-blue-50/50"}`}>
          ...
          <div className={`flex items-center justify-between px-4 pt-2 border-b ${darkMode ? "border-slate-700" : "border-slate-200"}`}>
            ...
            {/* 🔥 NEW: Expand/Minimize Chat Button */}
            {activeTab === "ai" && (
              <button 
          ...
          <div className="p-3 sm:p-4">
            ...
            {activeTab === "ai" && (
              // 🔥 NEW: Dynamic height based on expansion state
                      ...
                      <div className={`max-w-[85%] p-3 rounded-2xl text-sm overflow-x-auto ${msg.role === 'user' ? (darkMode ? "bg-blue-600 text-white rounded-tr-sm" : "bg-blue-600 text-white rounded-tr-sm") : (darkMode ? "bg-slate-800 border border-slate-700 text-slate-200 rounded-tl-sm" : "bg-white border border-slate-200 text-slate-800 rounded-tl-sm")}`}>
                        ...
                        <ReactMarkdown 
                          components={{
                            ...
                            // 🔥 FIX: Render paragraphs as <div> to prevent <p><pre></p> hydration errors
                            p: ({node, ...props}) => <div className="mb-2 last:mb-0 leading-relaxed block" {...props} />,
                            ul: ({node, ...props}) => <ul className="list-disc ml-4 mb-2" {...props} />,
                            ol: ({node, ...props}) => <ol className="list-decimal ml-4 mb-2" {...props} />,
                            // 🔥 FIX: Properly handle inline vs block code
