...
const AddProblemModal = ({ onAdd, isEdit = false, initialData = null, triggerElement = null, onClose = null }) => {
  const { darkMode } = useTheme();
  ...
  useEffect(() => {
    if (initialData && (open || isEdit)) {
      setForm({
        ...
        solutions: initialData.solutions?.length > 0 
          ? initialData.solutions 
          : initialData.solution 
            ? [{ title: "Solution", code: initialData.solution, language: "javascript" }]
  ...
  const addSolutionBlock = () => {
    setForm(prev => ({
      ...prev,
      solutions: [...prev.solutions, { title: `Approach ${prev.solutions.length + 1}`, code: "", language: "javascript" }]
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || err.message || "Failed to process problem.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleClose = () => {
    ...
    if (!isEdit) {
      setForm({
        question: "", description: "", solutions: [{ title: "Approach 1", code: "", language: "javascript" }],
        difficulty: "Medium", category: "", tags: [], time_complexity: "", space_complexity: "",
      });
      setTagSearch("");
    }
  };
    ...
    <>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
          <div className={`w-full max-w-2xl rounded-2xl shadow-xl animate-fadeUp my-8 max-h-[90vh] overflow-y-auto ${darkMode ? "bg-slate-800 border border-slate-700" : "bg-white border border-slate-200"}`}>
            
            <div className={`sticky top-0 z-10 flex items-center justify-between px-6 py-4 border-b ${darkMode ? "bg-slate-800/95 border-slate-700" : "bg-white/95 border-slate-100"}`}>
              <h2 className={`text-xl font-bold ${darkMode ? "text-white" : "text-slate-900"}`}>
                {isEdit ? "Edit Problem" : "Add New Problem"}
              </h2>
              <button onClick={handleClose} className={`p-2 rounded-xl transition-colors ${darkMode ? "text-slate-400 hover:bg-slate-700" : "text-slate-500 hover:bg-slate-100"}`}>
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div className="space-y-2">
                ...
                <div className="relative" ref={tagInputRef}>
                  <div className="relative">
                    <Search className={`absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 ${darkMode ? "text-slate-500" : "text-slate-400"}`} />
                    <input type="text" value={tagSearch} onChange={(e) => { setTagSearch(e.target.value); setShowTagDropdown(true); }} onFocus={() => setShowTagDropdown(true)} placeholder="Search to add tags..." className={`w-full pl-9 pr-4 py-2.5 rounded-xl border focus:ring-2 focus:ring-blue-500 outline-none transition-all ${darkMode ? "bg-slate-900 border-slate-700 text-white placeholder-slate-500" : "bg-slate-50 border-slate-200 text-slate-900 placeholder-slate-400"}`} />
                  </div>
                  {showTagDropdown && filteredTags.length > 0 && (
                    <div className={`absolute z-10 w-full mt-1 max-h-48 overflow-y-auto rounded-xl border shadow-lg ${darkMode ? "bg-slate-800 border-slate-700" : "bg-white border-slate-200"}`}>
                      {filteredTags.map((tag) => (
                        <button key={tag._id} type="button" onMouseDown={(e) => { e.preventDefault(); addTag(tag._id); }} className={`w-full text-left px-4 py-2 text-sm transition-colors ${darkMode ? "text-slate-200 hover:bg-slate-700" : "text-slate-700 hover:bg-slate-50"}`}>
                          {tag.name}
                        </button>
                      ))}
                    </div>
              ...
              <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                <div className="flex items-center justify-between">
                  <label className={`text-sm font-semibold ${darkMode ? "text-slate-300" : "text-slate-700"}`}>Solutions</label>
                  <button type="button" onClick={addSolutionBlock} className={`text-xs font-medium px-3 py-1.5 rounded-lg flex items-center gap-1 ${darkMode ? "bg-blue-900/30 text-blue-400 hover:bg-blue-900/50" : "bg-blue-50 text-blue-600 hover:bg-blue-100"}`}>
                    <Plus className="w-3 h-3" /> Add Approach
                  </button>
                </div>

                {form.solutions.map((sol, index) => (
                  <div key={index} className={`p-4 rounded-xl border ${darkMode ? "border-slate-700 bg-slate-800/50" : "border-slate-200 bg-slate-50"}`}>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-3">
                      ...
                      {form.solutions.length > 1 && (
                        <button type="button" onClick={() => removeSolutionBlock(index)} className="p-1.5 shrink-0 text-red-500 hover:bg-red-500/10 rounded-lg transition-colors">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-2">
                <div className="space-y-1">
                  <label className={`text-sm font-semibold ${darkMode ? "text-slate-300" : "text-slate-700"}`}>Time Complexity</label>
                  <select name="time_complexity" value={form.time_complexity} onChange={handleChange} className={`w-full px-4 py-2.5 rounded-xl border outline-none ${darkMode ? "bg-slate-900 border-slate-700 text-white" : "bg-slate-50 border-slate-200 text-slate-900"}`}>
                ...
                <div className="space-y-1">
                  <label className={`text-sm font-semibold ${darkMode ? "text-slate-300" : "text-slate-700"}`}>Space Complexity</label>
                  <select name="space_complexity" value={form.space_complexity} onChange={handleChange} className={`w-full px-4 py-2.5 rounded-xl border outline-none ${darkMode ? "bg-slate-900 border-slate-700 text-white" : "bg-slate-50 border-slate-200 text-slate-900"}`}>
                    <option value="">Select</option>
                    <option value="O(1)">O(1) - Constant</option>
                    <option value="O(log N)">O(log N) - Logarithmic</option>
                    <option value="O(N)">O(N) - Linear</option>
              ...
              <div className={`mt-6 pt-5 border-t flex items-center justify-end gap-3 ${darkMode ? "border-slate-700" : "border-slate-100"}`}>
                <button type="button" onClick={handleClose} className={`px-5 py-2.5 rounded-xl text-sm font-semibold transition-colors ${darkMode ? "text-slate-300 hover:bg-slate-700" : "text-slate-600 hover:bg-slate-100"}`}>
                  Cancel
                </button>
                <button type="submit" disabled={loading} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-xl shadow-md font-medium disabled:opacity-70 disabled:cursor-not-allowed">
                  {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
